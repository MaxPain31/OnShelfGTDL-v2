...
@section('content')
    <div
        x-data='{
            ...
            reserveBook() {
                ...
                // Check if user can reserve (has 5+ returned books)
                if (!this.selectedBook.can_reserve) {
            ...
            async confirmReserve() {
                ...
                try {
                    const csrfToken = document.querySelector("meta[name=csrf-token]")?.getAttribute("content");
                    const response = await fetch("{{ route('student.books.reserve', ':id') }}".replace(":id", this.selectedBook.id), {
                        method: "POST",
                        headers: {
                            "Accept": "application/json",
                            "X-Requested-With": "XMLHttpRequest",
    ...
    >
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <form id="filter-form" method="GET" action="{{ route('student.books') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-1 sm:max-w-2xl">
                {{-- Search Input --}}
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="pt-6 border-t border-[#f3cbe0] space-y-3">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            <button
                                                @click="reserveBook()"
                                                :disabled="selectedBook && (selectedBook.stock_quantity === 0 || !selectedBook.can_reserve)"
                                                ...
                                                :title="selectedBook && !selectedBook.can_reserve ? 'You need 5 successful borrows and returns to reserve books' : ''"
                                            >
