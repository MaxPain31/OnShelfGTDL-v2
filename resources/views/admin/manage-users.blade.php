...
@section('content')
    ...
    >
            ...
            <div class="rounded-[24px] border border-[#f3cbe0] bg-white">
                <div class="flex flex-wrap items-center gap-3 border-b border-[#f3cbe0] px-6 py-4">
                    <div class="flex items-center justify-between w-full ">
                        <h2 class="text-lg font-semibold text-[#4b2036]">Teacher Accounts</h2>
                        <div class="flex items-center gap-2">
                            <form method="GET" action="{{ route('admin.manage-teachers') }}" class="ml-auto relative">
                                <input
                                    type="search"
                                    name="search"
                    ...
                    <table class="min-w-full text-left text-sm text-[#4b2036]">
                        <thead class="bg-[#fde7f0] text-xs uppercase tracking-wider text-[#a03464]">
                            <tr>
                                ...
                                <th class="px-6 py-3 whitespace-nowrap">Advisory Class</th>
                                <th class="px-6 py-3 whitespace-nowrap">Email</th>
                                <th class="px-6 py-3 whitespace-nowrap">Mobile</th>
                                <th class="px-6 py-3 whitespace-nowrap">Status</th>
                                <th class="px-6 py-3 whitespace-nowrap">Last Login</th>
                                <th class="px-6 py-3 whitespace-nowrap">Created At</th>
                        ...
                        <tbody id="teacher-table-body" x-ref="teacherTableBody" class="divide-y divide-[#f7d6e6]">
                            @forelse (($teachers ?? collect()) as $teacher)
                                ...
                                <tr @class([$loop->odd ? 'bg-[#fff7fb]' : 'bg-white'])>
                                    <td class="px-6 py-3 font-semibold whitespace-nowrap">{{ $info->employee_number ?? '—' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $info->full_name ?? 'Unknown' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $info->advisory_class ?? '—' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $teacher->email }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $info->mobile ?? '—' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap"><span class="{{ $teacher->deactivated ? 'bg-rose-50 text-rose-700' : 'bg-green-50 text-green-700' }} px-2 py-1 text-xs font-semibold rounded-full">{{ $teacher->deactivated ? 'Inactive' : 'Active' }}</span></td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $teacher->last_login_at ? $teacher->last_login_at->format('M d, Y h:i A') : '—' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $teacher->created_at ? $teacher->created_at->format('M d, Y h:i A') : '—' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            <form id="status-form-{{ $teacher->id }}" method="POST" action="{{ route('admin.manage-teachers.status', $teacher) }}">
                                                ...
                                                <button
                                                    type="button"
                                                    class="flex h-8 w-8 items-center justify-center rounded-full border border-[#f3cbe0] text-[#a03464] hover:bg-[#fff2f8]"
                                            ...
                                            >
                                                ...
                                                <button
                                                    type="button"
                                                    class="flex h-8 w-8 items-center justify-center rounded-full border border-rose-200 text-rose-600 hover:bg-rose-50"
                                                    title="Delete"
                ...
                <div id="teacher-meta" x-ref="teacherMeta" class="border-t border-[#f3cbe0] px-6 py-4">
                    @php
                    ...
                    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between text-xs text-[#7c4c63]">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[#a03464] font-semibold">{{ $teachers->currentPage() }}</span>
                            <a
                                href="{{ $nextUrl ?: '#' }}"
                ...
                >
                    ...
                    >
                        <div class="flex items-center justify-between px-4 py-4 sm:px-6 sm:py-4">
                            <div>
                                <h2 class="text-xl sm:text-2xl font-semibold text-[#4b2036] leading-tight">Add Teacher</h2>
                            </div>
                            <button
                                type="button"
                        ...
                        <div class="px-4 pb-4 sm:px-6 sm:pb-6">
                            ...
                            <div class="flex justify-end gap-3 pt-4">
                                ...
                                <button
                                    type="submit"
                                    class="inline-flex items-center justify-center gap-2 rounded-[12px] bg-[#a03464] px-6 py-2 text-sm font-semibold text-white hover:bg-[#821a4f] disabled:cursor-not-allowed disabled:opacity-70"
            ...
            <template x-teleport="body">
                ...
                >
                    ...
                    >
                        <div class="flex items-center justify-between px-4 py-4 sm:px-6 sm:py-4">
                            <div>
                                <h2 class="text-xl sm:text-2xl font-semibold text-[#4b2036] leading-tight">Edit Teacher</h2>
                            </div>
                            <button
                                type="button"
                                class="rounded-full border border-[#f3cbe0] p-2 text-[#a03464] hover:bg-[#fff2f8]"
                        ...
                        <div class="px-4 pb-4 sm:px-6 sm:pb-6" x-show="editingTeacher">
                            <form
                                method="POST"
                        ...
                        <div x-show="!editingTeacher" class="px-4 pb-4 sm:px-6 sm:pb-6">
        ...
        @else
            ...
            <div class="rounded-[24px] border border-[#f3cbe0] bg-white">
                <div class="flex flex-wrap items-center gap-3 border-b border-[#f3cbe0] px-6 py-4">
                    <h2 class="text-lg font-semibold text-[#4b2036]">Student Accounts</h2>
                    <form method="GET" action="{{ route('admin.manage-students') }}" class="ml-auto relative" x-data="{ searchQuery: '{{ request('search', '') }}' }">
                        <input
                            type="search"
                            name="search"
                            x-model="searchQuery"
                    ...
                    <table class="min-w-full text-left text-sm text-[#4b2036]">
                        ...
                        <tbody class="divide-y divide-[#f7d6e6]">
                            @forelse (($students ?? collect()) as $student)
                                ...
                                @endphp
                                <tr @class([$loop->odd ? 'bg-[#fff7fb]' : 'bg-white'])>
                                    <td class="px-6 py-3 font-semibold whitespace-nowrap">{{ $info->lrn ?? '—' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $info->full_name ?? 'Unknown' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $student->email }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $info->mobile ?? '—' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $student->last_login_at ? $student->last_login_at->format('M d, Y h:i A') : '—' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ $student->created_at ? $student->created_at->format('M d, Y h:i A') : '—' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-6 text-center text-sm text-[#7c4c63]">
                ...
                <div id="student-meta" class="border-t border-[#f3cbe0] px-6 py-4">
                    @php
                    ...
                    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between text-xs text-[#7c4c63]">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[#a03464] font-semibold">{{ $students->currentPage() }}</span>
                            <a
                                href="{{ $nextUrl ?: '#' }}"
                                class="rounded-full border border-[#f3cbe0] px-3 py-1 text-xs font-semibold text-[#a03464] {{ $nextUrl ? 'hover:bg-[#fff2f8]' : 'opacity-60 cursor-not-allowed' }}"
                                @if(!$nextUrl) aria-disabled="true" @endif
