...
@section('content')
    <div
        x-data='{
            ...
            async openEbookModal(ebookId) {
                ...
                try {
                    const response = await fetch("{{ route('teacher.ebooks.show', ':id') }}".replace(":id", ebookId), {
                        method: "GET",
                        headers: {
            ...
            async favoriteEbook() {
                ...
                try {
                    ...
                    const response = await fetch("{{ route('teacher.ebooks.favorite', ':id') }}".replace(":id", this.selectedEbook.id), {
                        method: "POST",
                        headers: {
                            "Accept": "application/json",
                            "X-Requested-With": "XMLHttpRequest",
    ...
    >
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <form id="filter-form" method="GET" action="{{ route('teacher.ebooks') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-1 sm:max-w-2xl">
                {{-- Search Input --}}
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="pt-6 border-t border-[#f3cbe0] space-y-3">
                                        <template x-if="selectedEbook && selectedEbook.ebook_file_path">
                                            <a
                                                :href="'{{ route('teacher.ebooks.read', ':id') }}'.replace(':id', selectedEbook.id)"
